...
def addmovie():
    if request.method == 'POST':
        ...
        if existing_movie is None:
            ...
            for mov in movie:
                if mov[movie_name] == "movie_deleted":
                    user.update_many({}, {"$set": {movie_name: 0}})
                # else:
                #     if mov[movie_name] == 0:
                #         user.update_many({}, {"$set": {movie_name: 0}})
...
def deletemovie():
    if request.method == 'POST':
        ...
        movies = db.user_data.find({"$or": [{name: {"$lte": 2}}]})
        for mov in movies:
            user = mov["name"]
            db.user_data.update_one({"name": user}, {"$set": {name: "movie_deleted"}})
        if movie:
    ...
    if request.method == 'POST':
        ...
        if existing_user:
            if password1 == password2:
                ...
                users.update_one({"name": name}, {"$set": {"password": hashpass}})
                return jsonify({"status":"password updated "})

            return jsonify({"status":"password missmatch !"})
